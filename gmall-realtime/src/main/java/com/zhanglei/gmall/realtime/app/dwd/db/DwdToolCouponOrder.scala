...
//程序流程：Mock -> mysql(bin-log) -> Maxwell -> kafka(zookeeper) -> DwdToolCouponOrder -> kafka(zookeeper)
/***
 *  优惠卷使用（下单）事务事实表
 */
object DwdToolCouponOrder {
...
//    tableEnv.getConfig.setIdleStateRetention(Duration.ofSeconds(5))
    ...
    tableEnv.executeSql(MyKakfaUtil.getKafkaDB("dwd_tool_coupon_order"))

    //TODO 3.读取topic_db数据，过滤出优惠卷使用（下单）表数据
    ...
    tableEnv.executeSql(
      """
        ...
        |    using_time STRING
        |)
        |""".stripMargin + MyKakfaUtil.getKafkaSink("dwd_tool_coupon_order"))

    //TODO 5.将数据写入到kafka中
    tableEnv.executeSql("insert into dwd_tool_coupon_order select * from result_table")
      .print()
