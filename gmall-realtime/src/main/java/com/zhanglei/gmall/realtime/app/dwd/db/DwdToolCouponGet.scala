...
//程序流程：Mock -> mysql(bin-log) -> Maxwell -> kafka(zookeeper) -> DwdToolCouponGet -> kafka(zookeeper)
/***
 *  优惠卷领用事务事实表
 */
object DwdToolCouponGet {
...
//    tableEnv.getConfig.setIdleStateRetention(Duration.ofSeconds(5))
    ...
    tableEnv.executeSql(MyKakfaUtil.getKafkaDB("dwd_tool_coupon_get"))

    //TODO 3.读取topic_db数据，过滤出优惠卷领用表数据
    ...
    tableEnv.executeSql(
      """
        ...
        |    get_time STRING
        |)
        |""".stripMargin + MyKakfaUtil.getKafkaSink("dwd_tool_coupon_get"))
